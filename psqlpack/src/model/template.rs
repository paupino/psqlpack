...
fn assert_directory_exists(path: &Path) -> PsqlpackResult<()> {
    if path.exists() {
        ...
        if !path.is_dir() {
            bail!(TemplateGenerationError(format!("Provided output directory is either a file or does not have permissions for write: {}", path.to_str().unwrap())))
        }
    } else {
        // Create the directory tree
        fs::create_dir_all(path)
            .chain_err(|| TemplateGenerationError(format!("Failed to create directory tree: {}", path.to_str().unwrap())))?;
    }
    Ok(())
}
    ...
    File::create(path)
        ...
        .and_then(|file| {
            serde_json::to_writer_pretty(file, obj)
                .chain_err(|| TemplateGenerationError(format!("Failed to serialize JSON for file at: {}", path.to_str().unwrap())))
        })?;
    Ok(())
}
...
pub fn generate_project(output_path: &Path, name: &str) -> PsqlpackResult<()> {
    ...
    fs::create_dir(&output)
        .chain_err(|| TemplateGenerationError(format!("Failed to create project directory: {}", output.to_str().unwrap())))?;
